...
var VeeMoreGameBanner = VeeMoreGameButton.extend({
	...
	onCreate : function() {
		if (!vee.Ad.adBanner || !app.Config.PromoFullScreenEnable) {
...
var VeeQuitBox = vee.Class.extend({
	...
	onCreate : function() {
		...
		this.spAd.setScale(1.2);
		this.spAd.runAction(cc.EaseExponentialOut.create(cc.spawn(
			cc.scaleTo(0.5, 1),
			cc.fadeTo(0.5, 255)
		)));

		if (VeeQuitBox.enableDownload && vee.Ad.adEnabled) {
			...
			if (config) {
				...
				vee.Common.getInstance().downloadImage(config.image, _fileName, function(fileName){
					...
					if (jsb.fileUtils.isFileExist(fullPath)) {
						if (this.spAd) {
							this.spAd.initWithFile(fullPath);
						}
					} else {
...
var VeeMoreGameFullScreen = vee.Class.extend({
	...
	onCreate : function() {
		...
		this.spShow.setScale(1.2);
		this.spShow.setOpacity(0);
		this.spShow.runAction(cc.EaseExponentialOut.create(cc.spawn(
			cc.scaleTo(0.5, 1),
			cc.fadeTo(0.5, 255)
		)));
		this.randAd();
